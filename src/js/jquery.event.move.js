...
(function (module) {
	...
	function Timer(fn){
		...
		this.end = function(fnEnd) {
			var cb = callback;
			if (!fnEnd) { return; }
			if (!running) {
				fnEnd();
			} else {
				callback = active ? function(){ cb(); fnEnd(); } : fnEnd;
				active = true;
			}
		};
	}
	...
	function mousedown(e){
		...
		data = { target: e.target, startX: e.pageX, startY: e.pageY, timeStamp: e.timeStamp };
	...
	function triggerStart(e, template, touch, distX, distY, fn) {
		var touches = e.targetTouches, time = e.timeStamp - template.timeStamp;
	...
	function activeMouseend(e) {
		...
		endEvent(event, timer, function() {
			setTimeout(function(){ remove(event.target, 'click', returnFalse); }, 0);
		});
	}
	...
	jQuery.event.special.movestart = {
		...
		_default: function(e) {
			...
			data = { event: event, timer: new Timer(update), touch: undefined, timeStamp: undefined };
	...
	jQuery.event.special.move = {
		setup: function() { add(this, 'movestart.move', jQuery.noop); },
		teardown: function() { remove(this, 'movestart.move', jQuery.noop); }
	};
	jQuery.event.special.moveend = {
		setup: function() { add(this, 'movestart.moveend', jQuery.noop); },
		teardown: function() { remove(this, 'movestart.moveend', jQuery.noop); }
	};
